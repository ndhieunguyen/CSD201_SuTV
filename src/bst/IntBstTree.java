...
public class IntBstTree {
    ...
    private void printLRN(IntBstNode p){
        if (p!=null){
            printLRN(p.left);
            printLRN(p.right);
            System.out.println(p + ", ");
    ...
    public int getHeight(){
        class Node_Level {
            ...
            public Node_Level(IntBstNode p, int L) {
                node = p;
                level = L;
            }
        }
    ...
    private boolean removeOneChildNode(IntBstNode delNode){
        if (delNode==null || delNode.isLeaf() ||
            delNode.having2Children()) return false;
        if (delNode==root){
    ...
    private boolean deleteByMerging (IntBstNode delNode){
        ...
        if (delNode==root){
            ...
            // Connect grandFather <-> leftGrandChild
            // delNode is at the left side of grandFather
    ...
    public boolean deleteByMerging(int x){
        ...
        if (delNode.having1Child()) return removeOneChildNode(delNode);
    ...
    private boolean deleteByCopying(IntBstNode delNode) {
        ...
        if (rightMost.having1Child()) removeOneChildNode(rightMost);
        return true;
    }
